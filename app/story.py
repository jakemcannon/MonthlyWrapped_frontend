...
class Story:
    ...
    def create_header(self, draw, header_text, font_color):
        ...
        draw.text(
            ((self.W - text_width) / 2, settings.HEADER_POSITION_HEIGHT),
            header_text,
            font=font,
            fill=font_color,
        )
    ...
    def create_footer(self, draw, font_color):
        ...
        draw.text(
            (settings.FOOTER_POSITION), settings.FOOTER_TEXT, font=font, fill=font_color
        )
...
class SongStory(Story):
    ...
    def create_image(self):
        base = Image.new(
            "RGB", (self.W, self.H), ImageColor.getrgb(settings.SONG_BASE_COLOR)
        )
    ...
    def create_song_and_artist_text(self, draw):
        ...
        artist_font = ImageFont.truetype(
            settings.ARTIST_TEXT_FONT, settings.ARTIST_TEXT_SIZE
        )
        ...
        for i in range(10):
            ...
            if i < 9:
                draw.text(
                    settings.RANK_POSITION[i],
                    f"#{i+1}",
                    font=song_font,
                    fill=settings.SONG_STORY_FONT_COLOR,
                )
            else:
                draw.text(
                    settings.RANK_POSITION[i],
            ...
            draw.text(
                settings.SONG_TEXT_POSITION[i],
                self.songs[i],
                font=song_font,
                fill=settings.SONG_STORY_FONT_COLOR,
            )
            draw.text(
                settings.SONG_STORY_ARTIST_TEXT_POSITION[i],
                self.artists[i],
                font=artist_font,
                fill=settings.SONG_STORY_FONT_COLOR,
            )
...
class ArtistStory(Story):
    ...
    def create_image(self):
        base = Image.new(
            "RGB", (self.W, self.H), ImageColor.getrgb(settings.ARTIST_BASE_COLOR)
        )
        draw = ImageDraw.Draw(base)
    ...
    def create_artist_text(self, draw):
        artist_font = ImageFont.truetype(
            settings.SONG_TEXT_FONT, settings.SONG_TEXT_SIZE
        )
        ...
        for i in range(10):
            draw.text(
                ...
                f"#{i+1}",
                font=artist_font,
                fill=settings.ARTIST_STORY_FONT_COLOR,
            )
            draw.text(
                settings.ARTIST_TEXT_POSITION[i],
                self.artists[i],
                font=artist_font,
                fill=settings.ARTIST_STORY_FONT_COLOR,
            )
